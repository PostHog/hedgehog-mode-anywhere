...
import { HedgeHogMode, HedgehogModeRendererContent } from "@posthog/hedgehog-mode";
import { HedgehogModeUI } from "../vendor/hedgehog-mode/src/ui/GameUI";
...
// Mirror of the library's HedgehogModeRenderer, but with a synchronous ref guard.
// The upstream renderer guards engine setup on its async `game` state, so a re-render
// before that state commits initializes the engine (and spawns a hedgehog) twice.
function HedgehogRenderer({ config, onGameReady }) {
  const gameRef = useRef(null);
  const [game, setGame] = useState(null);

  const setup = (el) => {
    if (!el || gameRef.current) return;
    const instance = new HedgeHogMode(config);
    gameRef.current = instance;
    setGame(instance);
    instance.render(el).then(() => onGameReady?.(instance));
  };

  useEffect(() => () => gameRef.current?.destroy(), []);

  return (
    <HedgehogModeRendererContent id="hedgehog-mode-root">
      <div className="GameContainer" ref={setup} />
